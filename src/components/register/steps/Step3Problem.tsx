...
import { PROBLEM_STATEMENTS, MAX_TEAMS_PER_PROBLEM, type Chapter } from "@/lib/problemStatements";
...
export default function Step3Problem({
  ...
  const [chapter, setChapter] = useState<Chapter>("IEEE SSIT");
  ...
  const items = PROBLEM_STATEMENTS.filter((p) => p.chapter === chapter);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex gap-3">
          {(["IEEE SSIT", "IEEE ComSoc"] as Chapter[]).map((c) => (
            <button
              key={c} type="button" onClick={() => setChapter(c)}
              className={`rounded-full border px-4 py-1.5 text-sm ${chapter === c ? "border-cyan bg-cyan/10 text-cyan" : "border-white/10 text-white/60"}`}
            >
              {c}
            </button>
          ))}
