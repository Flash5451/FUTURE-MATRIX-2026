    ...
    <section id="problems" className="mx-auto max-w-6xl px-6 py-24">
      ...
      <div className="perspective-wrap mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {items.map((item, i) => {
          ...
          return (
            <button key={item.id} onClick={() => setActive(i)} className={`tilt-card rounded-xl border p-4 text-left transition-colors ${active === i ? "border-cyan bg-cyan/10" : "glass-panel hover:border-white/25"}`}>
